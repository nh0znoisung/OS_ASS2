...
static int translate(
	...
	//printf("ADDR:%d\tOFFSET: %d\tFirst: %d\tSecond: %d\n", virtual_addr, offset, first_lv, second_lv);	
	...
	for (i = 0; i < page_table->size; i++) {
		//printf("INDEX: %d\tSIZE: %d\n", page_table->table[i].p_index, page_table->size);
...
addr_t alloc_mem(uint32_t size, struct pcb_t * proc) {
	...
	if (mem_avail) {
		...
		for (int i = 0; i < NUM_PAGES; i++)
		{
			...
			{
				//printf("GIVE PAGE: %d for PROC: %d\n", i, proc->pid);
				needed_pages--; 
				_mem_stat[i].proc = proc->pid;
				_mem_stat[i].index = index_count;
				if (latest_page != -1) _mem_stat[latest_page].next = i;
				else first_page = i;
                                //printf("PAGE %d's next is %d\n", latest_page, _mem_stat[latest_page].next);
				index_count++;
				latest_page = i;
		...
		for (int i = 0; i < num_pages; i++)
		{
			...
			page_table->size++;
			//printf("v_index: %d\tp_index:%d\n", get_second_lv(current_addr), physical_index);
	...
	//printf("ALLOC: %d\tSIZE:%d\tPROC: %d\n", ret_mem, size, proc->pid);
	return ret_mem;
}
			...
			if(page_table->table[i].v_index == get_second_lv(address)){
				...
				if(translate(address, &physical_addr, proc)){
					...
					do{
						...
						int found = 0;
						cur_addr = (count_page_free << OFFSET_LEN) + address;
						seg_idx=get_first_lv(cur_addr);
						page_idx=get_second_lv(cur_addr);
							...
							if( proc->seg_table->table[k].v_index == seg_idx ){
								...
								for(l = 0; l < proc->seg_table->table[k].pages->size; l++){
									if(proc->seg_table->table[k].pages->table[l].v_index== page_idx){
										...
										//free page
										int size = proc->seg_table->table[k].pages->size;
										proc->seg_table->table[k].pages->table[l] = proc->seg_table->table[k].pages->table[size-1];
										proc->seg_table->table[k].pages->size--;

										if(proc->seg_table->table[k].pages->size == 0){//If all pages in segment are free
											free(proc->seg_table->table[k].pages);	//free page table
										
											//free segment(free all allocated page) 
											proc->seg_table->table[k] = proc->seg_table->table[proc->seg_table->size-1];
											proc->seg_table->size--;
										}
										found = 1;
										break;
